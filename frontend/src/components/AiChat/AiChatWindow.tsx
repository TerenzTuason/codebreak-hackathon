...
export default function AiChatWindow({ onClose }: AiChatWindowProps) {
  ...
  return (
    <div className="w-[500px] h-[600px] bg-white rounded-[24px] shadow-2xl flex flex-col overflow-hidden [&>button]:hidden">
      {/* Header */}
      <div className="bg-gradient-to-r from-blue-600 to-blue-500 p-4 text-white">
        <div className="flex items-center justify-between">
          ...
          <button onClick={onClose} className="hover:bg-white/10 rounded-full p-2 transition-all">
            <X className="h-5 w-5" />
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.map((message, index) => (
          ...
          >
            <div
              className={`max-w-[80%] rounded-[20px] p-3.5 shadow-sm ${
                message.isUser
      ...
      <div className="p-4 bg-gray-50/50 border-t border-gray-100">
        <div className="flex items-center gap-2 bg-white rounded-2xl p-1.5 shadow-sm ring-1 ring-gray-100">
          <input
            type="text"
            value={inputValue}
            ...
            className="flex-1 bg-transparent px-3 focus:outline-none text-sm"
          />
          <button
            onClick={handleSend}
